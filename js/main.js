...
// function to keep first slide from fixing until element reaches top
	...
	$(function(){   
			var itemOffset = fix.offset().top;

			$(window).scroll(function(){
			    let scrollTop = $(window).scrollTop();
			    if(scrollTop >= itemOffset){
			        fix.attr('class', 'cd-fixed-backgroun img-1 fix');
			    } else {
		    	fix.attr('class', 'cd-fixed-backgroun img-1');
		  		}
	});});
	...
	$(function () {
	  $(document).scroll(function () {
	  	...
	  	let div1Height = div.height();

	  	if ( scrollTop < (div1Height - 300) ) {
	  		menu.attr('class', 'nav raise icons');
	  	} 
	  	else if ( scrollTop > (div1Height - 300) ) {
	  		menu.attr('class', 'nav raise icons scrolled');
	  	}
